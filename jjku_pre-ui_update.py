...
map_image_file = Image.open("Images/city_streets_jjku_ss.JPG")
original_image = map_image_file
map_photo = ImageTk.PhotoImage(map_image_file)
canvas = tk.Canvas(root)
canvas.pack(fill = "both", expand = True)
map_image = canvas.create_image(0,0, anchor = "nw", image = map_photo)
canvas_image = map_photo

def old_ui_stuff():
    pieces = []
    p1 = Image.open("Pieces/Sorcerers/Naobito.png")
    p1_photo = ImageTk.PhotoImage(p1)
    p1_world_x = 500
    p1_world_y = 500
    player1 = canvas.create_image(p1_world_x,p1_world_y,anchor="center",image = p1_photo)
    ...
    def zoom_image(event):
        global zoom, photo
        global map_x, map_y
        mouse_map_x = (event.x - map_x) / zoom
        mouse_map_y = (event.y - map_y) / zoom
        if event.delta > 0:
            zoom *= 1.1
        else:
            zoom *= 0.9
        ...
        map_x = event.x - mouse_map_x * zoom
        map_y = event.y - mouse_map_y * zoom
        canvas.itemconfig(map_image, image=photo)
        canvas.coords(map_image, map_x, map_y)
        canvas.coords(
            player1,
            map_x + p1_world_x * zoom,
            map_y + p1_world_y * zoom
        )
        canvas.image = photo
        new_p1_width = int(p1.width * zoom)
        new_p1_height = int(p1.height * zoom)
        resized_p1 = p1.resize(
            (new_p1_width, new_p1_height),
            Image.Resampling.NEAREST
        )
        p1_photo = ImageTk.PhotoImage(resized_p1)
        canvas.itemconfig(player1, image=p1_photo)
        canvas.player1 = p1_photo
...
special_grade_deck: List[str] = [
    ...
    "Utahime (SG)", "Naoya (SG)", "Kenjaku (SG)", "Gojo (SG)", "Charles (SG)"
]

xp_drops: Dict[str, int] = {
    "G4": 1, "G3": 2, "G2": 3, "G1": 5, "FB": 10,
    "Toji (SG)": 16, "Seance Toji (SG)": 16, "Mahito (SG)": 17,
    "Awakened Mahito (SG)": 15, "Hanami (SG)": 20, "Jogo (SG)": 17,
    "Cursed Womb Dagon (SG)": 13, "Evolved Dagon (SG)": 19, "Sukuna (SG)": 21,
    "Teen Gojo (SG)": 17, "Todo (SG)": 14, "Megumi (SG)": 15, "Last Leg Megumi (SG)": 16,
    "Yuji (SG)": 16, "Inumaki (SG)": 17, "Geto (SG)": 18, "Teen Geto (SG)": 16, "Maki (SG)": 17,
    "Jiro (SG)": 12, "Utahime (SG)": 13, "Naoya (SG)": 20, "Kenjaku (SG)": 20, "Gojo (SG)": 22
}
...
binding_vow_deck: List[str] = [
    ...
    "Binding Vow of Debt", "Binding Vow of Selective Protection"
]
...
skill_card_deck: List[str] = [
    ...
    "Companionship", "Adrenaline Surge", "Ready"
]
...
squadron_cards: Dict[str, List[str]] = {
    ...
    "G4 Squadron II": ["5 G4s", "6 G4s", "7 G4s", "8 G4s", "9 G4s"],
    "G3 Squadron": ["3 G3s", "4 G3s", "5 G3s", "6 G3s", "8 G3s"],
    "G3 Squadron II": ["4 G3s", "5 G3s", "6 G3s", "7 G3s", "9 G3s"],
    "G2 Squadron": ["2 G2s", "3 G2s", "4 G2s", "4 G2s", "5 G2s"],
    "G2 Squadron II": ["3 G2s", "4 G2s", "5 G2s", "5 G2s", "6 G2s"],
    "G1 Squadron": ["1 G1", "1 G1", "1 G1", "2 G1s", "2 G1s"],
    "G1 Squadron II": ["1 G1", "1 G1", "2 G1", "2 G1s", "2 G1s"],
    "Finger Bearer Squadron": ["Redraw", "1 FB", "1 FB", "2 FB", "2 FB"],
    "Finger Bearer Squadron II": ["1FB", "1 FB", "1 FB", "2 FB", "2 FB"],
    "SG Squadron": ["Redraw", "50/50 1SG", "1 SG", "1 SG", "2 SGs"],
    "SG Squadron II": ["Redraw", "50/50 1SG", "1 SG", "1 SG", "2 SGs"]
}
...
sorcerer_data: Dict[str, Dict] = {
    "Yuji": {
        ...
        "move_points": 2,
        "attack_points": 3,
        "sorcerer_type": "Duelist",
        "dodge_requirement": "4+",
        "picture": "Pieces/Sorcerers/Yuji.png",
        "grades": {
            "G4": {
                "ce_max": 125,
                "ce_regen": 50,
            ...
            "ce_regen": 60,
            "additions": [
                {"attacks": {}, "perks": ["Resilient"]},
                {"attacks": {}, "perks": ["Soul Damage"]}
            ]
            },
            "G2": {
                "ce_max": 200,
                "ce_regen": 70,
                "additions": [
                    {"attacks": {"Black Flash": 40}, "perks": []},
                    {"attacks": {"Single Out": 55}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 250,
                "ce_regen": 85,
                "additions": [
                    {"attacks": {}, "perks": ["In the Zone"]},
                    {"attacks": {}, "perks": ["Up Close and Personal"]}
                ]
            },
            "SG": {
                "ce_max": 300,
    ...
    "Nanami": {
            ...
            "sorcerer_type": "Duelist",
            "dodge_requirement": "4+",
            "picture": "Pieces/Sorcerers/Nanami.png",
            "grades": {
                "G4": {
                    "ce_max": 125,
                    "ce_regen": 50,
                ...
                "G3": {
                    ...
                    "ce_regen": 60,
                    "additions": [
                        {"attacks": {"Forced Weakness": 55}, "perks": []},
                        {"attacks": {"Overhead Bash+": 40}, "perks": []}
                    ]
                },
                "G2": {
                    "ce_max": 200,
                    "ce_regen": 70,
                    "additions": [
                        {"attacks": {}, "perks": ["Responsibility of the Adult"]},
                        {"attacks": {}, "perks": ["Crippling Blow"]}
                    ]
                },
                "G1": {
                    "ce_max": 250,
                    "ce_regen": 85,
                    "additions": [
                        {"attacks": {}, "perks": ["Stagger"]},
                        {"attacks": {}, "perks": ["Black Flash"]}
                    ]
                },
                "SG": {
                    "ce_max": 300,
    ...
    "Todo": {
        ...
        "move_points": 2,
        "attack_points": 4,
        "sorcerer_type": "Disruptor",
        "dodge_requirement": "3+",
        "grades": {
            "G4": {
                "ce_max": 110,
                "ce_regen": 25,
                "attacks": {"Crippling Kick": 20},  # Damage via IRL dice rolls
                "perks": ["Disruptor Defense"]
            },
            "G3": {
                "ce_max": 200,
                "ce_regen": 45,
                "attacks": {"Crippling Kick": 20, "Boogie Woogie": 45},  # Damage via IRL dice rolls
                "perks": ["Disruptor Defense"]
            },
            "G2": {
                "ce_max": 250,
                "ce_regen": 85,
                "additions": [
                    {"attacks": {"Stone Strike": 60}, "perks": []},
                    {"attacks": {"Fake-Out": 60}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 300,
                "ce_regen": 100,
                "additions": [
                    {"attacks": {}, "perks": ["Teleport Support"]},
                    {"attacks": {}, "perks": ["Reflex Boogie"]}
                ]
            },
            "SG": {
                "ce_max": 400,
    ...
    "Gojo": {
            ...
            "grades": {
                ...
                "G3": {
                    ...
                    "ce_regen": 45,
                    "additions": [
                        {"attacks": {"Reversal Red": 50}, "perks": []},
                        {"attacks": {"Blue Sweep": 70}, "perks": []}
                    ]
                },
                "G2": {
                    "ce_max": 250,
                    "ce_regen": 85,
                    "additions": [
                        {"attacks": {"Long Range Teleport": 80}, "perks": []},
                        {"attacks": {"Blink": 60}, "perks": []}
                    ]
                },
                "G1": {
                    "ce_max": 300,
                    "ce_regen": 100,
                    "additions": [
                        {"attacks": {}, "perks": ["Infinity"]},
                        {"attacks": {"Hollow Purple": 150}, "perks": []}
                    ]
                },
                "SG": {
                    "ce_max": 400,
                    "ce_regen": 130,
                    "additions": [
                        {"attacks": {"Infinite Void": 300}, "perks": []}
                    ]
                }
            }
    },
    ...
    "Jogo": {
        ...
        "move_points": 3,
        "attack_points": 3,
        "sorcerer_type": "Ranger",
        "dodge_requirement": "5+",
        "grades": {
            "G4": {
                "ce_max": 130,
                "ce_regen": 30,
                "attacks": {"Disaster Flames": 30},
            ...
            "G3": {
                ...
                "ce_regen": 40,
                "additions": [
                    {"attacks": {}, "perks": ["Controlled Burn"]},
                    {"attacks": {}, "perks": ["Prideful Anger"]}
                ]
            },
            "G2": {
                "ce_max": 200,
                "ce_regen": 50,
                "additions": [
                    {"attacks": {"Volcano": 50}, "perks": []},
                    {"attacks": {"Ember Insects": 50}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 250,
                "ce_regen": 60,
                "additions": [
                    {"attacks": {"Maximum Meteor": 200}, "perks": []},
                    {"attacks": {"Contact Flame": 90}, "perks": []}
                ]
            },
            "SG": {
                "ce_max": 300,
                "ce_regen": 90,
                "additions": [
                    {"attacks": {"Coffin of the Iron Mountain": 250}, "perks": []}
                ]
            }
        }
    },
    "Dagon": {
        ...
        "grades": {
            ...
            "SG": {
                ...
                "ce_regen": 100,
                "additions": [
                        {"attacks": {"Horizon of the Captivating Skandha": 300}, "perks": []}
                    ]
            }
        }
    },
    "Sukuna": {
        ...
        "sorcerer_type": "Butcher",
        "dodge_requirement": "3+",
        "picture": "Pieces/Sorcerers/Sukuna.png",
        "grades": {
                "G4": {
                    "ce_max": 120,
                    "ce_regen": 30,
                ...
                "G2": {
                    ...
                    "ce_regen": 65,
                    "additions": [
                        {"attacks": {}, "perks": ["Thrill of Combat"]},
                        {"attacks": {"Cull the Herd": 100}, "perks": []}
                    ]
                },
                "G1": {
                    "ce_max": 250,
                    "ce_regen": 85,
                    "additions": [
                        {"attacks": {"Divine Flame": 150}, "perks": []},
                        {"attacks": {}, "perks": ["Real Jujutsu"]}
                    ]
                },
                "SG": {
                    "ce_max": 350,
                    "ce_regen": 100,
                    "additions": [
                        {"attacks": {"Malevolent Shrine": 300}, "perks": []}
                    ]
                }
            }
        },
    ...
    "Choso": {
            ...
            "sorcerer_type": "Ranger",
            "dodge_requirement": "5+",
            "picture": "Pieces/Sorcerers/Choso.png",
            "grades": {
                "G4": {
                    "ce_max": 130,
                    "ce_regen": 30,
                ...
                "G3": {
                    ...
                    "ce_regen": 40,
                    "additions": [
                        {"attacks": {"Blood Meteor": 40}, "perks": []},
                        {"attacks": {"Crimson Blade": 40}, "perks": []}
                    ]
                },
                "G2": {
                    "ce_max": 200,
                    "ce_regen": 50,
                    "additions": [
                        {"attacks": {}, "perks": ["Big Brother"]},
                        {"attacks": {}, "perks": ["Coagulated Armor"]}
                    ]
                },
                "G1": {
                    "ce_max": 250,
                    "ce_regen": 60,
                    "additions": [
                        {"attacks": {"Blood Meteor+": 50}, "perks": []},
                        {"attacks": {"Crimson Blade+": 50}, "perks": []}
                    ]
                },
                "SG": {
                    "ce_max": 300,
    ...
    "Hanami": {
            ...
            "sorcerer_type": "Tank",
            "dodge_requirement": "5+",
            "picture": "Pieces/Sorcerers/Hanami.png",
            "grades": {
                "G4": {
                    "ce_max": 80,
                    "ce_regen": 30,
    ...
    "Mahito": {
        ...
        "sorcerer_type": "Summoner",
        "dodge_requirement": "5+",
        "picture": "Pieces/Sorcerers/Mahito.png",
        "grades": {
            "G4": {
                "ce_max": 120,
                "ce_regen": 35,
                "attacks": {"Soul Harvest": 20},
                "perks": ["Summoner Dodge"]
            },
            "G3": {
                "ce_max": 160,
                "ce_regen": 50,
                "additions": [
                    {"attacks": {"Transfigured Human": 0}, "perks": []},
            ...
            "G2": {
                ...
                "ce_regen": 65,
                "additions": [
                        {"attacks": {"Polymorphic Soul Isomer": 0}, "perks": []},
            ...
            "G1": {
                ...
                "additions": [
                        ...
                        {"attacks": {"Soul Multiplicity": 60}, "perks": []}
                    ]
            },
            "SG": {
                "ce_max": 325,
    ...
    "Kamo": {
        ...
        "grades": {
            "G4": {
                "ce_max": 130,
                "ce_regen": 30,
                "attacks": {"Blood-Dipped Arrows": 15, "Free Arrow": 0},
                "perks": ["Ranger Rotation"]
            },
            "G3": {
                "ce_max": 150,
        ...
        "hp": 100,
            ...
            "grades": {
                "G4": {
                    "ce_max": 125,
                    "ce_regen": 50,
                    "attacks": {"Cursed Tool Combat": 25, "Follow-Up": 0},
                    "perks": ["Duelist Strike"]
                },
                "G3": {
                    "ce_max": 150,
    ...
    "Toji": {
            ...
            "sorcerer_type": "Butcher",
            "dodge_requirement": "3+",
            "picture": "Pieces/Sorcerers/toji.png",
            "grades": {
                "G4": {
                    "ce_max": 120,
                    "ce_regen": 30,
                    "attacks": {"Split Soul Katana": 40},  # Damage via IRL dice rolls
                    "perks": ["Butcher Budgeting"]
                },
                "G3": {
                    "ce_max": 175,
                ...
                "G2": {
                    ...
                    "ce_regen": 65,
                    "additions": [
                        {"attacks": {}, "perks": ["Invisible Man"]},
                        {"attacks": {"Flyhead Swarm": 75}, "perks": []}
                    ]
                },
                "G1": {
                    "ce_max": 250,
                    "ce_regen": 85,
                    "additions": [
                        {"attacks": {"Speed Blitz": 100}, "perks": []},
                        {"attacks": {}, "perks": ["Breach and Clear"]}
                    ]
                },
                "SG": {
                    "ce_max": 350,
                    "ce_regen": 100,
                    "additions": [
                        {"attacks": {"Chain of a Thousand Miles": 300}, "perks": []}
                    ]
                }
            }
        },
    "Nobara": { #Technically functional but sample system not established
            ...
            "grades": {
                ...
                "G3": {
                    ...
                    "ce_regen": 40,
                    "additions": [
                        {"attacks": {"Hairpin": 15}, "perks": []},
                        {"attacks": {}, "perks": ["Defensive Hairpin"]}
                    ]
                },
                "G2": {
                    "ce_max": 200,
    ...
    "Teen Geto": { #Technically functional, but inventory not set up
        ...
        "grades": {
            "G4": {
                "ce_max": 120,
                "ce_regen": 35,
                "attacks": {"Squid Shot": 20},
                "perks": ["Summoner Dodge"]
            },
            "G3": {
                "ce_max": 160,
                "ce_regen": 50,
                    "attacks": {"Squid Shot": 20,"Curse Summon": 25},
                    "perks": ["Summoner Dodge"],
            },
            "G2": {
                "ce_max": 190,
                "ce_regen": 65,
                "additions": [
                    {"attacks": {"Curse Summon+": 30}, "perks": []},
                    {"attacks": {"Stingray Escape": 30}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 230,
                "ce_regen": 75,
                "additions": [
                    {"attacks": {"Rainbow Dragon": 120}, "perks": []},
                    {"attacks": {"Worm Ambush": 100}, "perks": []}
                ]
            },
            "SG": {
                "ce_max": 325,
                "ce_regen": 90,
                "additions": [
                    {"attacks": {"Kuchisake-Onna": 255}, "perks": []},
                ]
            }
        }
    },
    "Naoya": {
            ...
            "sorcerer_type": "Duelist",
            "dodge_requirement": "4+",
            "picture": "Pieces/Sorcerers/Naoya.png",
            "grades": {
                "G4": {
                    "ce_max": 125,
                    "ce_regen": 50,
                ...
                "G2": {
                    ...
                    "additions": [
                        ...
                        {"attacks": {"Counterstrike": 70}, "perks": []}
                    ]
                },
                "G1": {
                    "ce_max": 250,
    ...
    "Naobito": {
            ...
            "sorcerer_type": "Duelist",
            "dodge_requirement": "4+",
            "picture": "Pieces/Sorcerers/Naobito.png",
            "grades": {
                "G4": {
                    "ce_max": 125,
                    "ce_regen": 50,
    ...
    "Revived Toji": {
            ...
            "grades": {
                ...
                "G2": {
                    ...
                    "ce_regen": 60,
                    "additions": [
                        {"attacks": {}, "perks": ["Enhanced Physicality"]},
                ...
                "G1": {
                    ...
                    "ce_regen": 70,
                    "additions": [
                        {"attacks": {"Playful Cloud Bash": 50}, "perks": []},
    ...
    "Higuruma": {
        ...
        "sorcerer_type": "Disruptor",
        "dodge_requirement": "3+",
        "picture": "Pieces/Sorcerers/Higuruma.png",
        "grades": {
            "G4": {
                "ce_max": 110,
                "ce_regen": 25,
                "attacks": {"Gavel Bash": 20},
                "perks": ["Disruptor Defense"]
            },
            "G3": {
                "ce_max": 200,
                "ce_regen": 45,
                "additions": [
                    {"attacks": {"Jury Slam": 45}, "perks": []},
                    {"attacks": {}, "perks": ["The Judgeman and His Executioner"]}
                ]
            },
            "G2": {
                "ce_max": 250,
                "ce_regen": 85,
                "additions": [
                    {"attacks": {}, "perks": ["Objection!"]},
                    {"attacks": {"Sustained": 0}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 300,
                "ce_regen": 100,
                "additions": [
                    {"attacks": {}, "perks": ["Evidence"]},
                    {"attacks": {"Retrial": 100}, "perks": []}
                ]
            },
            "SG": {
                "ce_max": 400,
                "ce_regen": 130,
                "additions": [
                    {"attacks": {"Deadly Sentencing": 300}, "perks": []}
                ]
            }
        }
    },
    "Ryu": {
        ...
        "sorcerer_type": "Ranger",
        "dodge_requirement": "5+",
        "picture": "Pieces/Sorcerers/Ryu.png",
        "grades": {
            "G4": {
                "ce_max": 130,
                "ce_regen": 30,
                "attacks": {"Granite Blast": 20},
                "perks": ["Ranger Rotation"]
            },
            "G3": {
                "ce_max": 150,
                "ce_regen": 40,
                "additions": [
                    {"attacks": {}, "perks": ["Environmental Destruction"]},
                    {"attacks": {}, "perks": ["Deep CE Reserve"]}
                ]
            },
            "G2": {
                "ce_max": 200,
                "ce_regen": 50,
                "additions": [
                    {"attacks": {"Boxing Ring": 40}, "perks": []},
                    {"attacks": {"Precise Shot": 75}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 250,
                "ce_regen": 60,
                "additions": [
                    {"attacks": {"Hair Comb": 90}, "perks": []},
                    {"attacks": {"Every Last Drop": 0}, "perks": []}
                ]
            },
            "SG": {
                "ce_max": 300,
                "ce_regen": 90,
                "additions": [
                    {"attacks": {"Binding Feast": 275, "Domain Granite Blast": 20}, "perks": []}
                ]
            }
        }
    },
    "Yuta": {
        ...
        "sorcerer_type": "Flex",
        "dodge_requirement": "4+",
        "picture": "Pieces/Sorcerers/Yuta.png",
        "grades": {
            "G4": {
                "ce_max": 110,
                "ce_regen": 25,
                "attacks": {"Katana": 20},
                "perks": ["Cursed Storage"]
            },
            "G3": {
                "ce_max": 175,
                "ce_regen": 45,
                "additions": [
                    {"attacks": {"Rika Lunge": 45}, "perks": []},
                    {"attacks": {"Rika Bind": 30}, "perks": []}
                ]
            },
            "G2": {
                "ce_max": 220,
                "ce_regen": 85,
                "additions": [
                    {"attacks": {}, "perks": ["Selection"]},
                    {"attacks": {"Cursed Propulsion": 95}, "perks": []}
                ]
            },
            "G1": {
                "ce_max": 275,
                "ce_regen": 100,
                "additions": [
                    {"attacks": {"Give Me Everything": 200}, "perks": []}
                ]
            },
            "SG": {
                "ce_max": 300,
                "ce_regen": 130,
                "additions": [
                    {"attacks": {"Authentic Mutual Love": 240}, "perks": []}
                ]
            }
        }
    },
...
class Sorcerer:
    def __init__(self, name: str):
        ...
        self.sorcerer_type: int = data["sorcerer_type"]
        ...
        self.soul_stock: int = 0
        self.used_free_hit = False
        # Sukuna-specific attributes
        self.sukuna_domain_turns: int = 0
        # Ryu-specific attributes
        self.boxing_ring: bool = False
        self.ryu_domain_turns: int = 0
        self.hair_comb_factor: int = 0
    ...
    def remove_ce(self) -> None:
        if self.is_knocked_out:
            print(f"{self.name} is knocked out and cannot lose/gain CE this turn.")
            return
        print(f"{self.name}’s Current CE: {self.ce}/{self.ce_max}")
        try:
            amount = int(input("Enter CE to remove/add (or 0 to cancel): "))
            self.ce += amount
            print(f"Added/removed {amount} CE to/from {self.name}. New CE: {self.ce}/{self.ce_max}")
        except ValueError:
            print("Invalid input. Enter a number.")
        ...
        try:
            ...
            if 0 <= choice < len(available_attacks):
                ...
                if self.ce < cost:
                    ...
                    return
                if attack == "Horizon of the Captivating Skandha": #MESSED UPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
                    if self.dagon_domain_active == True:
                    ...
                    if answer == "y":
                        if self.ce >= 150:
                            ...
                            sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["perks"].append("Death Swarm")
                            return
                        else:
                            print(f"Insufficient CE. Need 150, have {self.ce}.")
                            return
                    elif answer == "n":
                        ...
                        if answer == "y":
                            ...
                            self.ce = self.ce_max  # Set CE to max after cost deduction
                            print(
                                f"{self.name} uses Horizon of the Captivating Skandha! -{cost / 2} CE, CE reset to max (current: {self.ce}/{self.ce_max}), "
                                f"-1 attack point (current: {self.attack_points_current}/{self.attack_points}).")
                            print("Domain active for 4 turns: CE at max, dodge requirement lowered to 2+, Death Swarm available, Disaster Metabolism deactivated. Closes if CE dips below 40.")
                            sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["perks"].append("Death Swarm")
                            return
                ...
                if attack in ["Shikigami Summon", "Shikigami Summon+"]:
                    ...
                    print(f"{self.name} activates Chimera Shadow Garden! Targets in 1-tile radius cannot move for 2 turns, attack limit removed, shikigami costs halved, dead shikigami revived. -{cost} CE (current: {self.ce}/{self.ce_max})" +
                          (f", -1 attack point (current: {self.attack_points_current}/{self.attack_points})"
                           if self.attack_points_current < self.attack_points else ""))
                else:                                                                                      #This chunk Manages everyone's CE cost and AP... idk what's up with the Megumi line but I don't wanna touch it
                    self.ce -= cost
                    self.attack_points_current -= 1 if not (self.name == "Megumi" and self.domain_turns > 0) else 0
                    damage = 0
                    if self.name == "Dagon":
                ...
                if attack == "Curse Summon":
                    try:
                        self.ce += 25
                        num_spawns = int(input(f"How many fodder curses to spawn for {self.name}? (25 CE each, {self.ce} CE available): "))
                        total_cost = 25 * num_spawns
                        if num_spawns < 0:
                            print("Invalid number of spawns. Must be non-negative.")
                            return
                        if total_cost > self.ce:
                            print(f"Not enough CE to spawn {num_spawns}. Curses ({total_cost} CE needed, {self.ce} available).")
                            return
                        self.ce -= total_cost
                        if total_cost == 0:
                            print("Would you like to spawn a SG for 10 CE more? (y/n)")
                            answer = input()
                            if answer == "y":
                                self.ce -= 35
                            if answer == "n":
                ...
                if attack == "Curse Summon+":
                    try:
                        self.ce += 35
                        num_spawns = int(input(f"How many fodder curses to spawn for {self.name}? (35 CE each, {self.ce} CE available): "))
                        total_cost = 35 * num_spawns
                        if num_spawns < 0:
                            print("Invalid number of spawns. Must be non-negative.")
                            return
                        if total_cost > self.ce:
                        ...
                        print("Invalid input. Enter a number.")
                        return
                if attack == "Stingray Escape":
                    num_allies = int(input("How many allies would you like to take with you? (30 CE per ally)"))
                    total_cost = num_allies * 30
                    if num_allies < 0:
                        print("Invalid number of allies. Must be non-negative.")
                        return
                    if total_cost > self.ce:
                        print(f"Not enough CE to Bring that many allies. {self.ce} available).")
                        return
                    self.ce -= total_cost
#Yuji Itadori attacks
                if attack == "Divergent Fist":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        if damage == secondary:
                            print(f"Black Flash! Doubles rolled, so Divergent Fist deals {(max(damage, secondary)) * 2} damage!")
                        else:
                            print(f"Divergent Fist deals {(max(damage, secondary))} damage!")
                    elif q == "2":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        if damage == secondary:
                            print(f"Black Flash! Doubles rolled, so Divergent Fist deals {((max(damage, secondary)) + ((min(damage, secondary)) / 2)) * 2} damage!")
                        else:
                            print(f"Divergent Fist deals {(max(damage, secondary)) + ((min(damage, secondary)) / 2)} damage!")
                if attack == "Black Flash":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    call1 = input("Call a number (1-6):")
                    call2 = input("Call another number (1-6):")
                    calls = {call1, call2}
                    ...
                    matches = calls & dice
                    if matches:
                        print(f"Black FLash! Attack deals {d1 + d2 + d3} damage.")
                    else:
                        print(f"Black Flash failed. Attack deals {d1 + (d2 / 2)} damage.")
                if attack == "Single Out":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        ...
                        print(f"Single Out deals {damage} damage! Injured fodder pushed back 1 tile with Yuji.")
                    elif q == "2":
                        damage = random.randint(1, 6)
                        print(f"Single Out deals {damage} damage! Injured foe pushed back 1 tile with Yuji.")
                        next_damage = random.randint(1, 6)
                        print(f"Next hit deals {next_damage} damage!")
                        while self.move_points != 0:
                            answer = input("Keep going? Costs 1 MP, ends if you hit a wall or target dodges. (y/n):")
                            if answer == "y":
                                next_damage = random.randint(1, 6)
                                print(f"Single Out chain deals {next_damage} damage!")
                                self.move_points -= 1
                            elif answer == "n":
                                print("Single Out chain ends.")
                                break
                            else:
                                print("Invalid input. Please retry.")
                        if self.move_points == 0:
                            print("Out of MP. Chain over.")
                    else:
                        print("Misinput invalidates attack. AP and CE refunded. Please try again.")
                        self.attack_points += 1
                        self.ce += 55
                if attack == "Chain Combo":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    while 1:
                        ...
                        damage = sum(rolls)
                        print(f"Chain Combo deals {damage} damage! ({rolls})")
                        if len(set(rolls)) < 3:
                            print("Duplicate found! Chain Combo continues.")
                            continue
                        print("All values unique. Performing reroll check...")
                        reroll = random.randint(1, 6)
                        print(f"Reroll: {reroll}")
                        if reroll in rolls:
                            print("Match found! Chain Combo continues.")
                            continue
                        print("No matches found. Chain Combo ends.")
                        break
#Kento Nanami attacks
                if attack == "Overhead Bash":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    if "Black Flash" in self.perks:
                        call = int(input("Call a number (1-6):"))
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        damage = random.randint(1,6)
                        if "Black Flash" in self.perks:
                            if call == damage:
                                print("Black Flash! Damage output doubles!")
                                damage *= 2
                        if self.overtime_turns != 0:
                            print("Overtime adds +2 damage!")
                            damage += 2
                        print(f"Overhead Bash deals {damage} damage!")
                    elif q == "2":
                        damage = random.randint(1, 6)
                        if damage > 2:
                            damage_new = damage * 2
                        ...
                        if "Black Flash" in self.perks:
                            if call == damage:
                                ...
                                damage_new *= 2
                        if self.overtime_turns != 0:
                            print("Overtime adds +2 damage and guarantees hit!")
                            damage_new += 2
                        print(f"Overhead Bash deals {damage_new} damage!")
                if attack == "Forced Weakness":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    if "Black Flash" in self.perks:
                    ...
                    if q == "1":
                        ...
                        if self.overtime_turns != 0:
                            print("Overtime adds +2 damage!")
                            damage += 2
                        print(f"Forced Weakness makes curses inherit stats of grade level below them and deals {damage} damage!")
                    if q == "2":
                        damage = random.randint(1, 6)
                        if "Black Flash" in self.perks:
                            if call == damage:
                        ...
                        if self.overtime_turns != 0:
                            print("Overtime adds +2 damage and guarantees hit!")
                            damage += 2
                        print(f"Forced Weakness deals {damage} damage! Nanami's attacks and defense against this foe are buffed.")
                if attack == "Overhead Bash+":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    if q == "1":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        damage_new = (max(damage, secondary))
                        if "Black Flash" in self.perks:
                            if call == damage or call == secondary:
                                print("Black Flash! Damage output doubles!")
                                damage_new *= 2
                        if self.overtime_turns != 0:
                            print("Overtime adds +2 damage!")
                ...
                if attack == "Overtime":
                    ...
                    print(f"{self.name} activates Overtime! -4 damage taken, +2 to all attacks, and all attacks are guaranteed to hit for 3 turns.")
#Maki Zenin attacks
                if attack == "Cursed Tool Combat":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        print(f"Cursed Tool Combat deals {(max(damage, secondary))} damage!")
                    if q == "2":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        secondary_new = secondary / 2
                        secondary_new = math.floor(secondary_new)
                        print(f"Cursed Tool Combat deals {damage + secondary_new} damage!")
                if attack == "Follow-Up":
                    self.attack_points_current += 1
                    if self.used_free_hit:
                        print("Follow-Up already used this turn.")
                    else:
                        damage = random.randint(1, 6)
                        print(f"Follow-Up deals {damage} damage!")
                if attack == "Disarm":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    if q == "1":
                        ...
                        secondary = random.randint(1, 6)
                        if secondary > damage:
                            damage, secondary = secondary, damage
                        print(f"Disarm deals {damage} damage! All fodder in Maki's tile deal half damage to her allies for 1 turn.")
                    if q == "2":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        if secondary > damage:
                            damage, secondary = secondary, damage
                        print(f"Disarm deals {damage} damage! Targeted foe deals 3 less damage (min. 2). Can stack 3x.")
                if attack == "Joint Assault":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Stuff")
...
#Naobito Zenin attacks
                if attack == "Frame Rush":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        ...
                        secondary = random.randint(1, 6)
                        damage_new = (max(damage, secondary))
                        if damage == 6 or secondary == 6:
                            print("6 rolled! Extra die added.")
                            damage_new = damage_new + random.randint(1, 6)
                        print(f"Frame Rush deals {damage_new} damage!")
                    if q == "2":
                        damage = random.randint(1, 6)
                        secondary = random.randint(1, 6)
                        damage_new = (max(damage, secondary))
                        ...
                        if damage == 6 or secondary == 6:
                            ...
                            damage_new = (max(damage, secondary))
                            print(f"Frame Rush deals {damage_new} damage!")
                            print("6 rolled! Free Frame Rush performed.")
                if attack == "Freeze Frame":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    enemy_roll = random.randint(1, 6)
                    naobito_roll = random.randint(1, 6) + 3
                    if naobito_roll >= enemy_roll:
                        print("Enemy frozen for 1 turn! Allies in range 1 get to enter tile and attack for free.")
                    else:
                        print("Freeze Frame failed!")
                if attack == "Close the Distance":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    ...
                    if q == "2":
                        ...
                        if damage == 6 or secondary == 6:
                            ...
                            damage_new = (max(damage, secondary))
                            print(f"Frame Rush deals {damage_new} damage!")
                            print("6 rolled! Frame Rush is a guaranteed hit.")
                if attack == "Frame Cancel":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    naobito_sg_voiceline = pygame.mixer.Sound("Audio/Naobito_SG_Voiceline.wav")
                    naobito_sg_voiceline.play()
                    roll1 = random.randint(1, 6)
                    roll2 = random.randint(1, 6)
                    print(f"Rolls: (1) {roll1} (2) {roll2}")
                    added = input("Choose which roll gains +2. (1 or 2):")
                    if added == 1:
                        roll1+= 2
                    if added == 2:
                        roll2 += 2
                    choose = input("Choose which will determine how many attacks you can cancel in range 1. Other will determine damage. (1 or 2):")
                    if choose == 1:
                        print(f"Cancel up to {roll1} attacks. Deal {roll2} damage to each enemy you cancel.")
                    if choose == 2:
                        print(f"Cancel up to {roll2} attacks. Deal {roll1} damage to each enemy you cancel.")
#Naoya Zenin attacks
                if attack == "Hailstorm":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    damage = random.randint(1, 6)
                    if "Momentum" in self.perks:
                        ask = input("Did Naoya move to this target before attacking? (y/n):")
                        if ask == "y":
                            momentum = random.randint(1, 6)
                            damage = damage + momentum
                            print(f"Momentum boosts damage by {momentum}!")
                        else:
                            print("Momentum does not boost damage.")
                    print(f"Hailstorm deals {damage} damage! Three consecutive hailstorms on a single target stun!")
                if attack == "Charge Up":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        count = int(input("How many tiles did Naoya travel?"))
                        for tile in range(1, count + 1):
                            damage = random.randint(1, 6)
                            print(f"Tile {tile}: {damage} damage.")
                    if q == "2":
                        count = int(input("How tiles did Naoya travel?"))
                        rolls = []
                        for _ in range(count):
                            rolls.append(random.randint(1, 6))
                            damage = sum(rolls)
                            print(f"Naoya travels {count} tiles and deals {damage} damage!")
                if attack == "Paused Strike":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    number = random.randint(1, 24)
                    if number != 24:
                        print("Attack is guaranteed to hit!")
                    else:
                        print("Attack can be dodged.")
                    roll1 = random.randint(1, 6)
                    roll2 = random.randint(1, 6)
                    while 1:
                        print(f"Results: {roll1} | {roll2}")
                        menu = input("Reroll? 20CE per reroll")
                        print(f"1 -- {roll1}")
                        print(f"2 -- {roll2}")
                        print("3 -- Attack")
                        if menu == "1":
                            if self.ce < 20:
                                print("Cannot reroll. Insufficient CE. Proceed with attack.")
                            self.ce = self.ce - 20
                            roll1 = random.randint(1, 6)
                        elif menu == "2":
                            if self.ce < 20:
                    ...
                    print(f"Paused Strike deals {roll1 * roll2} damage!")
#Satoru Gojo attacks
                if attack == "Lapse Blue":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        damage = random.randint(1, 6)
                        targets = int(input("How many fodder targets? "))
                        for target in range(1, targets + 1):
                            move = random.randint(1, 6)
                            if move >= 3:
                                print(f"Lapse Blue moves target #{target}!")
                        print(f"Lapse Blue deals {damage} damage!")
                    if q == "2":
                        damage = random.randint(1, 6)
                        targets = int(input("How many FB/SG targets? "))
                        for target in range(1, targets + 1):
                            move = random.randint(1, 6)
                            if move >= 3:
                                print(f"Lapse Blue moves target #{target} 1 tile!")
                        print(f"Lapse Blue deals {damage} damage per target!")
                if attack == "Reversal Red":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        damage = random.randint(1, 6)
                        if damage >= 4:
                            print("Surviving fodder is pushed back 1 tile!")
                        print(f"Reversal Red deals {damage} damage!")
                    if q == "2":
                        print(f"Reversal Red deals {damage} damage and pushes target back! Can push through walls.")
                if attack == "Blue Sweep":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print(f"Blue Sweep deals {damage} damage per target! SGs and FBs can dodge.")
                if attack == "Hollow Purple":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    damage = random.randint(1, 6) + random.randint(1, 6)
                    print(f"Hollow Purple does {damage} damage to 3 tiles in a row!")
                if attack == "Infinite Void":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    self.infinite_void_turns = 2
                    print(f"{self.name} activates Infinite Void! Domain active for 2 turns.")
#Aoi Todo attacks

#Toge Inumaki attacks
                if attack == "Evasive Maneuvers":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    roll1 = random.randint(1, 6)
                    roll2 = random.randint(1, 6)
                    print("Pick which roll to determine evasion (4+ success). Leftover will deal damage.")
                    evasion = input(f"Choose: {roll1} | {roll2} (1 or 2): ")
                    if evasion == "1":
                        print(f"Evasive Maneuvers deals {roll2} damage!")
                        if roll1 >= 4:
                            print("Inumaki gets a free MP to use immediately.")
                    if evasion == "2":
                        print(f"Evasive Maneuvers deals {roll1} damage!")
                        if roll2 >= 4:
                            print("Inumaki gets a free MP to use immediately.")
                if attack == "Stop":
                    if self.attack_points_current < 0:
                        ...
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    stun = random.randint(1, 6)
                    if q == "1":
                        print(f"'Stop' stuns {stun} targets! 50% of their XP value is awarded.")
                    if q == "2":
                        if stun == 1:
                            print("Enemy resists! Stun attempt fails.")
                        else:
                            print("Enemy stunned for 1 turn! Award 1 G1 of XP.")
                            print("If enemy is 2 grades higher, lose 3HP. If 3 grades higher, lose 5HP.")
                if attack == "Run Away":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Allies in target tile get 1 free MP to exit with. Ignore enemies.")
                if attack == "Get Back":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    if q == "1":
                        if roll1 >= 2 and roll2 >= 2:
                            print(f"'Get Back' deals {(max(roll1, roll2))} damage!")
                        print("All targeted fodder are knocked back 1 tile.")
                    if q == "2":
                        if roll1 >= 3 and roll2 >= 3:
                            print(f"'Get Back' deals {(max(roll1, roll2))} damage!")
                        print("Targeted foe is knocked back 2 tiles. Hitting a wall removes an MP for next turn.")
                if attack == "Help Me":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    self.hp -= 1
                    print("Enemy will automatically fight for Inumaki for 1 turn. -1HP. HP cost doubles per repossession of same enemy.")
                if attack == "Explode":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("All G4 and G3 curses in range 1 instantly die.")
#Hiromi Higuruma attacks
                if attack == "Gavel Bash":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    damage = random.randint(1, 6)
                    question = input(f"Gavel Bash deals {damage} damage! Would you like to increase range by 1 for -2 damage? (y/n): ")
                    if question == "y":
                        damage -= 2
                        if damage <= 0:
                            damage = 1
                        print(f"Range expanded! {damage} damage dealt.")
                    if question == "n":
                ...
                if attack == "Jury Slam":
                    ...
                    damage = random.randint(1, 6)
                    if q == "1":
                        allies = int(input("How many allies are in Higuruma's tile? "))
                        print(f"Jury Slam deals {damage + (allies * 2)} damage!")
                    if q == "2":
                        allies = int(input("How many allies are in Higuruma's tile? "))
                        rolls = [random.randint(1, 6) for _ in range(allies)]
                        damage = max(rolls)
                        print(f"Rolls: {rolls}")
                        print(f"Jury Slam deals {damage} damage!")
                if attack == "Sustained":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Sustained! The next ally that attacks will have a guaranteed hit effect!")
                if attack == "Retrial":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("WE'RE HAVING A RETRIAL!")
                    higuruma_sg_voiceline_retrial = pygame.mixer.Sound("Audio/Higuruma_SG_Voiceline_Retrial.wav")
                    higuruma_sg_voiceline_retrial.play()
                    sentence = random.randint(1, 6)
                    if "Evidence" in self.perks:
                        sentence += 1
                    if sentence < 5:
                        print("GUILTY. CONFISCATION.")
                        print("Targeted enemy cannot use any perks for 2 turns. All hazards and summons are dispelled. Domain closes.")
                    if sentence >= 5:
                        print("GUILTY. DEATH PENALTY.")
                        print("Higuruma's Gavel Bash is now a one-hit kill if it hits the target of the domain. Domain closes.")
                if attack == "Deadly Sentencing":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    higuruma_sg_voiceline = pygame.mixer.Sound("Audio/Higuruma_SG_Voiceline.wav")
                    higuruma_sg_voiceline.play()
                    print("Domain Expansion: Deadly Sentencing opens! Judgeman reviews the case...")
                    sentence = random.randint(1,6)
                    if "Evidence" in self.perks:
                        sentence += 1
                    if sentence < 5:
                ...
                if self.name == "Jogo":
                    if attack == "Disaster Flames":
                        if self.attack_points_current < 0:
                            ...
                            return
                        damage = random.randint(1, 6)
                        residual = random.randint(1, 6)
                        if residual > damage:
                            damage, residual = residual, damage
                        if self.jogo_domain_active:
                            damage += 1
                            residual += 1
                        print(f"{self.name} uses Disaster Flames!, "f"Flames deal {damage} damage, residual flames deal {residual} damage.")
                    elif attack == "Volcano":
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        self.place_volcano()
                        return
                    elif attack == "Ember Insects":
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        damage = random.randint(1, 6)
                        knockback = random.randint(1, 6)
                        if knockback > damage:
                            damage, knockback = knockback, damage
                        if self.jogo_domain_active:
                            damage += 1
                        print(f"Ember Insects deals {damage} damage!")
                        if knockback >= 3:
                            print(f"Ember Insects knocks enemies back 1 tile!")
                    elif attack == "Contact Flame":
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        damage = random.randint(1, 6) + random.randint(1, 6)
                        if self.jogo_domain_active:
                            damage += 1
                        print(
                            f"Contact Flame deals {damage} damage! Will continue to deal 1 damage per turn until death.")
                        ...
                        if self.jogo_domain_active:
                            damage += 1
                        print(f"{self.name} uses Maximum Meteor! Surviving SGs and FBs take {damage} damage.")
                        return
                    if attack == "Coffin of the Iron Mountain":
                        ...
                        if self.name != "Jogo" or self.grade != "SG":
                            print(
                                f"{self.name} cannot use Coffin of the Iron Mountain (requires Jogo at Special Grade).")
                            return
                        if self.jogo_domain_active:
...
#Choso Kamo attacks
                if attack == "Piercing Blood":
                    ...
                    if damage < secondary:
                        damage, secondary = secondary, damage
                    print(f"Piercing Blood deals {damage} damage and {secondary} damage to the tile behind it!")
                if attack == "Blood Meteor":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    ...
                    if q == "1":
                        damage *= 2
                        print(f"Blood Meteor deals {damage} damage!")
                    if q == "2":
                        damage *= 1.5
                        damage = math.floor(damage)
                        print(f"Blood Meteor deals {damage} damage!")
                if attack == "Crimson Blade":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    damage = random.randint(1, 6)
                    ...
                    if damage < secondary:
                        damage, secondary = secondary, damage
                    print(f"Crimson Blade deals {damage} damage! Target loses 2HP per turn until death or healed.")
                    if secondary >= 4:
                        print("Choso may exit the tile for free.")
                if attack == "Blood Meteor+":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    if q == "1":
                        damage *= 2
                        print(f"Blood Meteor set! Will deal {damage} damage upon detonation!")
                    if q == "2":
                        damage *= 1.5
                        damage = math.floor(damage)
                        print(f"Blood Meteor set! Will deal {damage} damage upon detonation!")
                if attack == "Blood Rain":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    choso_sg_voiceline = pygame.mixer.Sound("Audio/Choso_SG_Voiceline.wav")
                    choso_sg_voiceline.play()
                    damage = random.randint(1, 6) + random.randint(1, 6)
                    print(f"Blood Rain deals {damage} damage per outdoor target!")
#Nobara Kugisaki attacks

#Noritoshi Kamo attacks
                if attack == "Blood-Dipped Arrows":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        self.ce += 15 #for cancelling out initial cost
                        num_arrows = int(input(f"How many arrows to fire? (1–3, 15 CE each, {self.ce} CE available, 0 to cancel): "))
                        if num_arrows == 0:
                            print("Blood-Dipped Arrows cancelled.")
                            return
                        if num_arrows < 0 or num_arrows > 3:
                            print("Invalid number of arrows. Must be non-negative.")
                            self.attack_points_current += 1  # refunds AP
                            return
                        if num_arrows > 3:
                            print("Too many arrows to fire at once. Choose 1–3 arrows.")
                            self.attack_points_current += 1  # refunds AP
                            return
                        total_cost = 15 * num_arrows
                        if total_cost > self.ce:
                            print(f"Not enough CE to fire {num_arrows} arrows ({total_cost} CE needed, {self.ce} available).")
                            self.attack_points_current += 1 #refunds AP
                            return
                        self.ce -= total_cost
                        print(f"{self.name} fires {num_arrows} Blood-Dipped Arrows! -{total_cost} CE (current: {self.ce}/{self.ce_max})")
                        for arrow in range(1, num_arrows + 1):
                            print(f"Arrow {arrow} deals {random.randint(1, 6)} damage!")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        return
                if attack == "Free Arrow":
                    self.attack_points_current += 1 #refunds lost AP
                    damage = random.randint(1, 6)
                    print(f"Free Arrow deals {damage} damage!")
                if attack == "Blood-Dipped Arrows+":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                        ...
                        for arrow in range(1, num_arrows + 1):
                            ...
                            if second >= 5:
                                print(f"Target of Arrow {arrow} loses 1 MP!")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        return
                if attack == "Maim":
                    if self.attack_points_current < 0:
                        ...
                        return
                    damage = random.randint(1, 6)
                    print(f"Main deals {damage} damage! The target must now roll 2 5+ dice to dodge instead of a single 6.")
                if attack == "Crimson Binding":
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        print("All fodder in target tile stunned for 1 turn if equal or higher to Kamo in grade level or 2 turns if lower than Kamo.")
                    if q == "2":
                        print("Target is stunned for a turn! Must pass dodge check twice to dodge incoming attacks! Kamo gets 25% XP if an ally kills a bound target.")
                if attack == "Slicing Exorcism":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    damage = random.randint(1, 6)
                    print(f"Slicing Exorcism deals {damage} damage to every fodder enemy it crosses. Will stop upon reaching an SG or FB, or hitting a wall.")
                if attack == "Flowing Red Scale":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    if self.kamo_red_scale_turns > 0:
                        print("Flowing Red Scale already active.")
                        return
                    if self.kamo_red_scale_cooldown > 0:
                        print(f"Flowing Red Scale is on cooldown for {self.kamo_red_scale_cooldown} more turns.")
                        return
                    if self.kamo_red_scale_cooldown == 0 and self.kamo_red_scale_turns == 0:
                        self.ce = self.ce_max
                        self.kamo_red_scale_turns = 2
                        self.kamo_red_scale_cooldown = 5  # 2 turns active + 3 turns cooldown
                        print(f"{self.name} uses Flowing Red Scale! CE restored to max ({self.ce}/{self.ce_max}), active for 2 turns.")
                        print("Kamo's dodge requirement is lowered to 3+ and he automatically deals 2DMG (undodgeable) to all enemies in his tile at the start of each turn FRS is active.")
#Ryu Ishigori attacks
                if attack == "Granite Blast":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print(f"BOXING_RING = {self.boxing_ring}")
                    if self.boxing_ring == True:
                        first = random.randint(1, 6)
                        second = random.randint(1, 6)
                        print(f"Granite Blast (melee variant) deals {first + second} damage!")
                    elif self.boxing_ring == False:
                        q = input("Fodder or foe variant? (1 or 2):")
                        damage = random.randint(1, 6)
                        if q == "1":
                            damage = random.randint(1, 6)
                            ...
                            if secondary < 1:
                                secondary = 1
                            if self.hair_comb_factor != 0:
                                print(f"Granite Blast deals {damage * self.hair_comb_factor} damage and {secondary * self.hair_comb_factor} damage is dealt to the tile behind it!")
                                self.hair_comb_factor = 0
                            else:
                                print(f"Granite Blast deals {damage} damage and {secondary} damage is dealt to the tile behind it!")
                        if q == "2":
                            damage = random.randint(1, 6)
                            if self.hair_comb_factor != 0:
                                print(f"Granite Blast deals {damage * self.hair_comb_factor} damage!")
                                self.hair_comb_factor = 0
                            else:
                                print(f"Granite Blast deals {damage} damage!")
                if attack == "Boxing Ring":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    first = random.randint(1, 6)
                    second = random.randint(1, 6)
                    print(f"Boxing Ring deals {first + second} damage! All subsequent Granite Blast attacks this turn will be melee only and all incoming damage is reduced by 50%.")
                    self.boxing_ring = True
                if attack == "Precise Shot":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    rolls = [random.randint(1, 6) for _ in range(3)]
                    rolls.sort(reverse=True)
                    print(f"Precise Shot deals {rolls[0] + rolls[1]} damage and affects {rolls[2]} tiles behind the target!")
                if attack == "Hair Comb":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    factor = random.randint(1, 6)
                    print(f"If Ryu follows up with a Granite Blast, its resulting damage will be multiplied by {factor}!")
                    self.hair_comb_factor = factor
                if attack == "Every Last Drop":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    dice_count = self.ce // 60
                    self.ce -= self.ce
                    rolls = [random.randint(1, 6) for _ in range(dice_count)]
                    damage = sum(rolls)
                    larp = random.randint(1, 50)
                    if larp == 25:
                        print("LET'S LARP.")
                    print(f"Every Last Drop deals {damage} damage to target tile and {round(damage / 2)} damage to all tiles behind it!")
                if attack == "Domain Granite Blast":
                    if self.ryu_domain_turns == 0:
                        print("Cannot fire Domain Granite Blast outside of Binding Feast. Cost refunded.")
                        self.ce += 20
                        self.attack_points_current += 1
                    elif self.ryu_domain_turns > 0:
                        self.attack_points_current += 1
                        if self.boxing_ring == True:
                            self.boxing_ring = False
                            print("At this point... WORDS ARE MEANINGLESS! Boxing ring disabled for domain.")
                        damage = random.randint(1, 6)
                        print(f"Granite Blast (domain variant) deals {damage} damage to 3 tiles in a line!")
                if attack == "Binding Feast":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ryu_sg_voiceline = pygame.mixer.Sound("Audio/Ryu_SG_Voiceline.wav")
                    ryu_sg_voiceline.play()
                    self.ryu_domain_turns = 2
                    g4_g3 = int(input("How many G4s and G3s in your domain? "))
                    g2 = int(input("How many G2s? "))
                    g1 = int(input("How many G1s? "))
                    fb = int(input("How many FBs? "))
                    sg = int(input("How many SGs? "))
                    ce_load = (g4_g3 * 10) + (g2 * 15) + (g1 * 20) + (fb * 30) + (sg * 50)
                    self.ce += ce_load
#Mechamaru attacks

#Iori Hazenoki attacks

#Toji Fushiguro attacks
                if attack == "Split Soul Katana":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    ...
                    second = random.randint(1, 6)
                    if second > damage:
                        damage, second = second, damage
                    if q == "1":
                        answer = input("Was an MP used to reach this target? (y/n): ")
                        if answer == "y":
                            print(f"Split Soul Katana deals {damage + second} damage!")
                        if answer == "n":
                            print(f"Split Soul Katana deals {damage} damage!")
                    if q == "2":
                        print(f"Split Soul Katana deals {damage} damage! Target can never heal from this attack.")
                    if "9mm pistol" in self.perks:
                        kill = input("Did Toji kill any targets? (y/n) ")
                        if kill == "y":
                            shoot = input("Do you want to eliminate a G4/G3 in range 1 for 5CE? (y/n) ")
                            if shoot == "y":
                                ask = int(input("How many?"))
                                self.ce -= (5 * ask)
                            else:
                                print("Target(s) killed.")
                            if shoot == "n":
                                print("No targets shot.")
                        if kill == "n":
                            print("No targets shot.")
                if attack == "Playful Cloud":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    while 1:
                        counter = 0
                        damage = random.randint(1, 6)
                        print(f"Playful Cloud deals {damage} damage!")
                        answer = input("Did this attack result in any kills? (y/n): ")
                        if answer == "y":
                            counter += 1
                        elif answer == "n":
                            print("Playful Cloud chain broken.")
                            break
                        else:
                            print("Invalid input. Roll invalid.")
                        if counter >= 5:
                            print("Playful Cloud chain maxxed out! Chain ends.")
                            break
                    if "9mm pistol" in self.perks:
                        kill = input("Did Toji kill any targets? (y/n) ")
                        if kill == "y":
                            ...
                            if shoot == "y":
                                ...
                                if self.ce < 0:
                                    self.ce += (5 * ask)
                                    print("Not enough CE. CE refunded.")
                                else:
                                    print("Target(s) killed.")
                            if shoot == "n":
                                print("No targets shot.")
                        if kill == "n":
                            print("No targets shot.")
                if attack == "Flyhead Storm":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Flyhead Storm cast! Toji cannot be attacked or incur CE tax in a 1 tile radius for 1 turn. Allies also do not incur CE tax.")
                if attack == "Speed Blitz":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    count = int(input("How many tiles did Toji travel?"))
                    for tile in range(1, count + 1):
                        damage = random.randint(1, 6)
                        damage2 = random.randint(1, 6)
                        print(f"Tile {tile}: {damage + damage2} damage.")
                    if "9mm pistol" in self.perks:
                        kill = input("Did Toji kill any targets? (y/n) ")
                        if kill == "y":
                            shoot = input("Do you want to eliminate a G4/G3 in range 1 for 5CE? (y/n) ")
                        ...
                        if kill == "n":
                            print("No targets shot.")
                if attack == "Chain of a Thousand Miles":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    toji_sg_voiceline = pygame.mixer.Sound("Audio/Toji_SG_Voiceline.wav")
                    toji_sg_voiceline.play()
                    d1 = random.randint(1, 6)
                    d2 = random.randint(1, 6)
                    d3 = random.randint(1, 6)
                    final = (d1 + d2 + d3) * 2
                    print(f"Chain of a Thousand Miles deals {math.ceil(final)} damage across a 2 tile radius! Distribute as seen fit. Any surviving enemies may be drawn into Toji's tile.")
                    if "9mm pistol" in self.perks:
                        kill = input("Did Toji kill any targets? (y/n) ")
                        if kill == "y":
                            shoot = input("Do you want to eliminate a G4/G3 in range 1 for 5CE? (y/n) ")
...
#Ryomen Sukuna attacks
                if attack == "Cleave":
                    ...
                    if q == "2":
                        print(f"Cleave deals {damage} damage!")
                if attack == "Point-Blank":
                    q = input("Fodder or foe variant? (1 or 2):")
                    damage = random.randint(1, 6)
                    second = random.randint(1, 6)
                    if q == "1":
                        print(f"Point-Blank deals {damage + second} damage!")
                    if q == "2":
                        if second > damage:
                            damage, second = second, damage
                        print(f"Point-Blank deals {damage} Damage! Will still deal {math.floor(damage)} if dodged.")
                if attack == "Cleave+":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    if q == "1":
                        ...
                        d2 = random.randint(1, 6)
                        d3 = random.randint(1, 6)
                        print(f"Cleave+ deals {d1 + d2 + d3} damage!")
                    if q == "2":
                        rolls = [random.randint(1, 6) for _ in range(3)]
                        rolls.sort(reverse=True)
                        print(f"Cleave+ deals {rolls[0] + rolls[1]} damage!")
                        if rolls[2] >= 3:
                            print("Sukuna's next attack on this enemy is a guaranteed hit!")
                if attack == "Cull the Herd":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        kill_count = 0
                        count = int(input("How many fodder enemies in Sukuna's tile?"))
                        for target in range(1, count + 1):
                            damage = random.randint(1, 6)
                            if damage >= 3:
                                kill_count += 1
                        print(f"Sukuna kills {kill_count} fodder enemies!")
                    if q == "2":
                        count = int(input("How many SGs or FBs in target's tile?"))
                        for target in range(1, count + 1):
                            damage = random.randint(1, 6)
                            if damage < 3:
                                damage += 1
                if attack == "Divine Flame":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    d1 = random.randint(1, 6)
                    d2 = random.randint(1, 6)
                    print(f"Divine Flame deals {d1 + d2 + 6} guaranteed damage! Residual fire lasts 2 turns and deals 2 damage to all who enter it.")
                if attack == "Malevolent Shrine":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    d1 = random.randint(1, 6)
                    ...
                    if d2 > d1:
                        d1, d2 = d2, d1
                    print(f"Malevolent Shrine opens! ALL targets caught in the domain take {d1 + 2} damage per turn. Active for 2 turns. Sukuna's dodge check is temporarily lowered to 2+ and is stuck in the middle of his domain until it closes.")
                    self.sukuna_domain_turns = 2
                    if self.sukuna_domain_turns > 0:
                        print("Domain already active. Refunding AP and CE.")
                        self.attack_points_current += 1
                        self.ce += 300
# Dagon attacks

# Revived Toji Fushiguro attacks
                if attack == "Hard Hit":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    ask = input("Is Toji in a hostile domain? (y/n): ")
                    if ask == "y":
                        damage = first + second
                        print(f"Hard Hit deals {damage} damage!")
                    if ask == "n":
                        print(f"Hard Hit deals {first} damage!")
                if attack == "Rush":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    ...
                    rolls = [random.randint(1, 6) for _ in range(3)]
                    rolls.sort(reverse=True)
                    print(f"Toji travels {rolls[0]} tiles and deals {rolls[1] + rolls[2]} damage!")
                if attack == "Defensive Leap":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Toji leaps to his ally!")
                if attack == "Playful Cloud Bash":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    q = input("Fodder or foe variant? (1 or 2):")
                    if q == "1":
                        ...
                        if second > first:
                            first, second = second, first
                        print(f"Toji deals {first * 2} damage!")
                    if q == "2":
                        first = random.randint(1, 6)
                        second = random.randint(1, 6)
                        print(f"Toji deals {first + second} damage! Toji may move himself and his enemy 1 tile in any cardinal direction.")
                if attack == "Parry Stance":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Parry Stance activated! Grant Toji 1 XP per 2 DMG taken next enemy turn cycle.")
                if attack == "The One Who Left it All Behind":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    print("Toji enters the hostile domain, ignoring its sure-hit effect!")
                    first = random.randint(1, 6)
                    second = random.randint(1, 6)
                    damage = first + second
                    print(f"Hard Hit deals {damage} damage!")
# Mahito attacks
                if attack == "Transfigured Human":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        num_spawns = int(input(f"How many Transfigured Humans to spawn for {self.name}? (15 CE each, {self.ce} CE available): "))
                        total_cost = 15 * num_spawns
                        if num_spawns < 0:
                            print("Invalid number of spawns. Must be non-negative.")
                            self.attack_points += 1
                            return
                        if total_cost > self.ce:
                            print(f"Not enough CE to spawn {num_spawns} Transfigured Humans ({total_cost} CE needed, {self.ce} available).")
                            self.attack_points_current += 1
                            return
                        self.soul_stock = self.soul_stock - num_spawns
                        if self.soul_stock < 0:
                            print("Not enough souls in stock. Cannot spawn that many transfigured humans.")
                            self.soul_stock = self.soul_stock + num_spawns
                            self.attack_points_current += 1
                            return
                        self.ce -= total_cost
                        print(f"{self.name} spawns {num_spawns} Transfigured Humans! -{total_cost} CE (current: {self.ce}/{self.ce_max})")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        self.attack_points_current += 1
                        return
                if attack == "Soul Harvest":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        hp_heal = int(input(f"How much HP to heal for {self.name} using Swift Slash? (Max {self.hp_max - self.hp}): "))
                        if hp_heal < 0:
                        ...
                        if self.ce < 20:
                            print(f"Not enough CE for Swift Slash (20 CE needed, {self.ce} available).")
                            self.attack_points_current += 1
                            return
                        self.hp = min(self.hp + hp_heal, self.hp_max)
                        self.ce -= 20
                        print(f"{self.name} uses Swift Slash, dealing {damage} damage and healing {hp_heal} HP! -50 CE (current: {self.ce}/{self.ce_max})")
                        soul_gain = int(input("How many enemies were killed?"))
                        self.soul_stock = soul_gain
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        self.attack_points_current += 1
                        return
                if attack == "Idle Transfiguration":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        ...
                        self.attack_points_current += 1
                        return
                if attack == "Polymorphic Soul Isomer":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        num_spawns = int(input(f"How many Polymorphic Soul Isomers to spawn for {self.name}? 50 CE each, {self.ce} available."))
                        self.soul_stock = self.soul_stock - (num_spawns * 3)
                        if self.soul_stock < 0:
                            print("Not enough souls in stock. Cannot spawn that many isomers.")
                            self.soul_stock = self.soul_stock + num_spawns
                            self.attack_points_current += 1
                            return
                        self.ce -= num_spawns * 50
                        print(f"{self.name} spawns {num_spawns} Polymorhpic Soul Isomers!")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        self.attack_points_current += 1
                        return
                if attack == "Soul Multiplicity: Body Repel":
                    try:
                        if self.attack_points_current < 0:
                            print(f"{self.name} has no attack points remaining this turn.")
                            return
                        self.soul_stock -= 3
                        if self.soul_stock < 0:
                            print("Not enough souls in stock. Cannot spawn body repel.")
                            self.soul_stock += 3
                            self.attack_points_current += 1
                            return
                        print(f"{self.name} deploys soul multiplicity: body repel!")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        self.attack_points_current += 1
                        return
                if attack == "Decoy":
                    try:
                        ...
                        self.soul_stock -= 1
                        if self.soul_stock < 0:
                            print("Not enough souls in stock. Cannot spawn decoy.")
                            self.soul_stock += 1
                            self.attack_points_current += 1
                            return
                        print(f"{self.name} spawns a decoy!")
                    except ValueError:
                        print("Invalid input. Enter a number.")
                        self.attack_points_current += 1
                        return
                if attack == "Domain Expansion: Self-Embodiment of Perfection":
                    if self.attack_points_current < 0:
                        print(f"{self.name} has no attack points remaining this turn.")
                        return
                    if self.ce < 275:
                    ...
                    print(f"{self.name} uses Domain Expansion: Self-Embodiment of Perfection! +7 HP for 2 turns. -300 CE (current: {self.ce}/{self.ce_max})")
                    soul_gain = int(input("How many enemies were killed?"))
                    self.soul_stock = soul_gain
    ...
    def maki_rallying_cry(self):
        self.attack_points_current += self.attack_points_current
        ...
        if self.name == "Dagon":
            ...
            if self.dagon_domain_active:
                if self.ce < 40:
                    ...
                    sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["perks"].remove("Death Swarm")
            ...
            if "Death Swarm" in self.perks:
                sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["attacks"]["Piranhas"] = 40
                sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["attacks"]["Eel"] = 60
                sorcerer_data["Dagon"]["grades"]["SG"]["additions"][0]["attacks"]["Giant Isopod"] = 100
                print("Placeholder")
    ...
    def take_damage(self, damage: int) -> bool:
        ...
        if self.name == "Nanami" and self.overtime_turns > 0:
            print(f"Overtime active ({self.overtime_turns} remain): Subtract 4 from damage taken.")
    ...
    def gain_ce(self) -> None:
        if self.is_knocked_out:
            ...
            return
        if "The Judgeman and His Executioner" in self.perks:
            ask = input("Is there a Duelist in your tile? (y/n): ")
            if ask == "y":
                regen = self.ce_regen * 1.20
            if ask == "n":
                regen = self.ce_regen
        else:
            regen = self.ce_regen
        if "Higuruma" in sorcerer_pool and self.sorcerer_type == "Duelist":
            ask = input("Is Higuruma in your tile? (y/n): ")
            if ask == "y":
                regen = self.ce_regen * 1.10
            if ask == "n":
                regen = self.ce_regen
        else:
            regen = self.ce_regen
        if self.name == "Ryu" and "Deep CE Reserve" in self.perks:
            regen = self.ce_regen * 1.2
    ...
    def level_up(self) -> None:
        thresholds = {"G4": 10, "G3": 35, "G2": 75, "G1": 100}
        ...
        while True:
            ...
            grade_data = data["grades"][self.grade]
            self.ce_max = grade_data["ce_max"]
            self.ce_regen = grade_data["ce_regen"]
            if "additions" in grade_data:
                additions = grade_data["additions"]
                if len(additions) > 1:
                    # Player choice for middle grades
                    print(f"\n{self.name} promotes to {self.grade}! Choose additions:")
                    for i, option in enumerate(additions, 1):
                        attacks_str = ', '.join(f'{k} ({v})' for k, v in option["attacks"].items())
                        perks_str = ', '.join(option["perks"])
                        print(f"{i}) Attacks: {attacks_str or 'None'} | Perks: {perks_str or 'None'}")
                    try:
                        choice = int(input("Select option (1 or 2): "))
                        if 1 <= choice <= len(additions):
                            selected_option = additions[choice - 1]
                        else:
                            print("Invalid choice. Using option 1.")
                            selected_option = additions[0]
                    except ValueError:
                        print("Invalid input. Using option 1.")
                        selected_option = additions[0]
                else:
                    # Single addition (e.g., SG) - auto-add
                    selected_option = additions[0]
                    print(f"\n{self.name} promotes to {self.grade}! Adding fixed abilities.")
                self.attacks.update(selected_option["attacks"])
                self.perks.extend(selected_option["perks"])
            else:
                # Fixed for G4 (replace)
                self.attacks = grade_data["attacks"]
                self.perks = grade_data["perks"]
    ...
    def show_info(self) -> None:
        ...
        if self.name == "Mahito":
            print(f"Soul Stock: {self.soul_stock} souls")
        if self.name == "Dagon":
        ...
        if self.binding_vows:
            vow_conditions = {
                ...
                "Binding Vow of Uneasy Peace": "Choose an enemy and restrict them to do nothing but move, but they can't move, either.",
                "Binding Vow of Leverage": "Next domain clash is an instant win, but the domain must close 1 turn early.",
                "Binding Vow of Debt": "Can domain counter regardless of CE amount, but they can't do anything but dodge for the turn after their domain closes."
            }
    ...
    def reset_turn_cycle(self) -> None:
        ...
        print(f"reset_turn_cycle() called for {self.name}")
        print(f"boxing_ring before reset = {self.boxing_ring}")
        if self.boxing_ring == True:
            self.boxing_ring = False
            print(f"boxing_ring after reset = {self.boxing_ring}")
            print("Ryu's Boxing Ring dissipates.")
        if self.hair_comb_factor != 0:
            self.hair_comb_factor = 0
            print("Hair comb bonus lost.")
    ...
    try:
        ...
        if enemy_type <= 5:
            ...
            for s in valid_sorcerers:
                if "The Judgeman and His Executioner" in s.perks:
                    nearby_duelist = input("Is there a Duelist in Higuruma's tile? (y/n): ").lower()
                    if nearby_duelist == "y":
                        xp_per_sorcerer = round(xp_per_sorcerer * 1.20)
                elif s.sorcerer_type == "Duelist":
                    higuruma_present = input(f"Is Higuruma in {s.name}'s tile? (y/n): ").lower()
                    if higuruma_present == "y":
                        xp_per_sorcerer = round(xp_per_sorcerer * 1.20)
            ...
            for s in sorcerer_pool:
                if s.name == "Higuruma":
                    nearby_duelist = input(
                        "Is there a Duelist in Higuruma's tile? (y/n): ").lower()
                    if nearby_duelist == "y":
                        xp_award = round(xp_award * 1.20)
                elif s.sorcerer_type == "Duelist":
                    higuruma_present = input(f"Is Higuruma in {s.name}'s tile? (y/n): ").lower()
                    if higuruma_present == "y":
                        xp_award = round(xp_award * 1.20)
            ...
            for s in sorcerer_pool:
                if s.name == "Yuji" and "Resilient" in s.perks and s.grade in ["G4", "G3", "G2", "G1", "SG"]:
                    attacks = int(input(f"How many successful attacks did {s.name} land on {sg}? "))
                    ce_gain = attacks * 15
                    if ce_gain > 0:
                        s.ce = min(s.ce + ce_gain, s.ce_max)
                        print(f"{s.name} gains {ce_gain} CE from Resilient perk "
...
def manage_sorcerer_pool():
    ...
    if not sorcerer_pool:
        sorcerer_names = [
            ...
            "Kamo", "Teen Geto", "Naoya", "Naobito", "Kurourushi",
            "Revitalized Todo", "Maki", "Kashimo", "Higuruma", "Takaba",
            "Miguel", "Uraume", "Ryu", "Mechamaru", "Showdown Sukuna",
            "Awakened Maki", "Unsealed Gojo", "Final Yuji", "Revived Toji",
            "Mei Mei", "True Body Mahito", "Yuki", "Yaga", "Hazenoki"
        ]
        print("\nAvailable Sorcerers:")
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                print("4) Manage Skill Cards")
                print("5) Use Binding Vow")
                print("6) Award XP")
                print("7) Perform Attack")
                print("8) Change CE")
                print("9) Regenerate CE")
                next_option = 10
                ...
                if action == "1":
                    ...
                    if assistant_action == "1":
                        if selected.assistant == "Shoko":
                            ...
                            print(f"{selected.assistant}’s ability is applied or always active: {assistant_abilities[selected.assistant]}")
                    ...
                    if action == "1":
                        ...
                        card_choice = 0
                        try:
                            card_choice = int(input("Select a skill card to give: ")) - 1
                            if 0 <= card_choice < len(selected.skill_cards):
                                card = selected.skill_cards.pop(card_choice)
                                   #don't forget thissssssss
                        except ValueError:
                            print("Invalid input. Enter a number.")
                        ...
                        if not other_sorcerers:
                            print("No other sorcerers available to receive the assistant.")
                            continue
                        print("\nSelect a sorcerer to receive the skill card:")
                        for i, s in enumerate(other_sorcerers, 1):
                            status = f" (has {s.skill_cards})" if s.skill_cards else ""
                            print(f"{i}) {s.name}{status}")
                        try:
                            choice = int(input("Choose a sorcerer (or 0 to cancel): ")) - 1
                            if choice == -1:
                                print("Give skill card cancelled.")
                                continue
                            if 0 <= choice < len(other_sorcerers):
                                target = other_sorcerers[choice]
                        ...
                        print("This shit don't work yet. Sorry.")
                        other_sorcerers = [s for s in sorcerer_pool if s != selected and s.skill_cards]
                        if not other_sorcerers:
                            print("No other sorcerers with skill cards available to swap with.")
                        else:
                            print("\nSelect a sorcerer to swap skill cards with:")
                            for i, s in enumerate(other_sorcerers, 1):
                                print(f"{i}) {s.name} ({len(s.skill_cards)} cards)")
                            try:
                                target_choice = int(input("Choose a sorcerer (or 0 to cancel): ")) - 1
                                if target_choice == -1:
                                    print("Swap cancelled.")
                                elif 0 <= target_choice < len(other_sorcerers):
                                    target = other_sorcerers[target_choice]

                                    # Select card from selected
                                    print(f"\n{selected.name}’s Skill Cards:")
                                    for i, card in enumerate(selected.skill_cards, 1):
                                        print(f"{i}) {card}")
                                    my_card_choice = int(input("Select your card to give: ")) - 1

                                    # Select card from target
                                    print(f"\n{target.name}’s Skill Cards:")
                                    for i, card in enumerate(target.skill_cards, 1):
                                        print(f"{i}) {card}")
                                    their_card_choice = int(input("Select their card to receive: ")) - 1

                                    if 0 <= my_card_choice < len(selected.skill_cards) and 0 <= their_card_choice < len(
                                            target.skill_cards):
                                        my_card = selected.skill_cards.pop(my_card_choice)
                                        their_card = target.skill_cards.pop(their_card_choice)
                                        selected.skill_cards.append(their_card)
                                        target.skill_cards.append(my_card)
                                        print(
                                            f"Swap complete! {selected.name} gave {my_card} and received {their_card} from {target.name}.")
                                    else:
                        ...
                        if not selected.skill_cards:
                            print("No skill cards to discard.")
                        else:
                            print(f"\n{selected.name}’s Skill Cards:")
                            for i, card in enumerate(selected.skill_cards, 1):
                                print(f"{i}) {card}")
                            try:
                                card_choice = int(input("Select a skill card to discard (or 0 to cancel): ")) - 1
                                if card_choice == -1:
                                    print("Discard cancelled.")
                                elif 0 <= card_choice < len(selected.skill_cards):
                                    card = selected.skill_cards.pop(card_choice)
                    ...
                    if sacrifice == "MP":
                        ...
                        print("Invalid input. Try again.")
                    if "Cough Syrup" in sorcerer.perks:
                        sorcerer.hp += 1
                elif action == str(10) and has_todo_sg_action:
                    print("Previewing Enemy Turn...")
                elif action == str(10 + int(has_todo_teleport)) and has_todo_teleport:
                    if selected.ce < 40:
                ...
                elif action == str(10 + int(has_todo_sg_action)) and has_megumi_unsummon:
                    ...
                    try:
                        ...
                        print("Invalid input. Enter a number.")
                elif action == str(10 + int(has_todo_sg_action) + int(has_todo_teleport)) and has_megumi_unsummon:
                    print(f"\n{selected.name}’s Active Shikigami:")
                    for i, shikigami in enumerate(selected.active_shikigami, 1):
                        print(f"{i}) {shikigami['name']} (HP: {shikigami['hp']})")
                    try:
                        ...
                        print("Invalid input. Enter a number.")
                elif action == str(10 + int(has_todo_sg_action) + int(has_todo_teleport) + int(
                        has_megumi_unsummon)) and has_ranger_rotation:
                    selected.use_ranger_rotation()
                elif action == str(
                        10 + int(has_todo_sg_action) + int(has_todo_teleport) + int(has_megumi_unsummon) + int(
...
def encounter_card_menu() -> None:
    ...
    if card == "Hidden Shrine":
        ...
        print(f"Prison Realm activated! {selected.name} is trapped, needs another sorcerer "
            ...
            for sorcerer in sorcerer_pool:
                ...
                if sorcerer.name == "Nanami" and sorcerer.overtime_turns > 0:
                    ...
                    if sorcerer.infinite_void_turns == 0:
                        print(f"{sorcerer.name}’s Infinite Void closes.")
                elif sorcerer.name == "Kamo" and sorcerer.kamo_red_scale_turns > 0:
                    sorcerer.kamo_red_scale_turns -= 1
                    print(f"{sorcerer.name}’s Flowing Red Scale active ({sorcerer.kamo_red_scale_turns} turns left).")
                    if sorcerer.kamo_red_scale_cooldown > 0:
                        print(f"{sorcerer.name}'s Flowing Red Scale is on cooldown. {sorcerer.kamo_red_scale_cooldown} turns left.")
                    else:
                        print(f"{sorcerer.name}’s Flowing Red Scale cooldown ended.")
                elif sorcerer.name == "Sukuna" and sorcerer.sukuna_domain_turns > 0:
                    sorcerer.sukuna_domain_turns -= 1
                    if sorcerer.sukuna_domain_turns == 0:
                        print(f"{sorcerer.name}'s Malevolent Shrine closes.")
                elif sorcerer.name == "Ryu" and sorcerer.ryu_domain_turns > 0:
                    sorcerer.ryu_domain_turns -= 1
                    if sorcerer.ryu_domain_turns == 0:
                        print(f"{sorcerer.name}'s Binding Feast closes.")
            sorcerer.reset_turn_cycle()
        elif choice == "8":
            print("Exiting...")
            break
        else:
